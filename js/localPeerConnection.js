...
function successCallback(stream){
    ...
    if (window.URL) {
        // localVideo.srcObject = URL.createObjectURL(stream);
...
function start() {
    ...
    // Enable the Stop button on the page
    stopButton.disabled = false;
}
function stop() {
    log("Ending local stream");
    // First of all, disable the Stop button on the page
    stopButton.disabled = true;
    if (!!localStream) {
        localVideo.srcObject = null;
        localStream.active = false;
    }
    // Enable the Start button on the page
    startButton.disabled = false;
}
...
function call() {
    ...
    callButton.disabled = true;
    // Disable the Stop button on the page
    stopButton.disabled = true;
